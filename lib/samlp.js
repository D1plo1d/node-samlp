...
function buildSamlResponse(options, callback) {
  ...
  if (options.signResponse) {
    ...
    var signatureAlgorithm = constants.ALGORITHMS.SIGNATURE[options.signatureAlgorithm]
    ...
    sig.computeSignature(cannonicalized, { prefix: options.signatureNamespacePrefix, location: { action: 'after', reference: "//*[local-name(.)='Issuer']" }}, function(err, sigXML) {
      if (err || !sigXML) {
        callback(err);
      } else {
        SAMLResponse = sigXML.getSignedXml();
...
function getSamlResponse(options, user, callback) {
  ...
  if (options.signatureFunction && typeof options.signatureFunction === 'function') {
    samlOptions.signatureFunction = options.signatureFunction;
  }

  saml20.create(samlOptions, function (err, signedAssertion) {
    if (err) return callback(err);

    options.signedAssertion = signedAssertion;
    options.samlStatusCode = options.samlStatusCode || constants.STATUS.SUCCESS;
    buildSamlResponse(options, callback);
  });
}
...
module.exports.sendError = function (options) {
  ...
  function renderResponse(res, postUrl) {
    ...
    buildSamlResponse(options, function(err, SAMLResponse){
      var response = new Buffer(SAMLResponse);
